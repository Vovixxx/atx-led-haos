...
3. Read `/dali/api/addresses` and `/dali/api/devices`. Best-effort GET `/dali/api/groups` and `/dali/api/scenes` must not fail setup if missing.
...
5. Derive capabilities from device flags, not names or merely populated color fields. Groups inherit color-temperature when member fixtures advertise it.
...
Use the stored device name as the initial display name (`hue_name` when the hub provides one). User renames in Home Assistant should persist. Hue discovery visibility (`hue_hidden`) must not prevent native integration discovery.
...
Expose only when `has_color_temp` is true, or for groups when at least one member fixture advertises it. The valid Kelvin range comes from `user_warm` / `user_cool` as mireds; group sliders use the intersection of member ranges. Fixture control uses `POST /dali/api/devices/{id}` with `color_temp_k`. Group Kelvin uses the same write on each color-temp member. Do not interchange physical/user mireds with kelvin or use arbitrary `color_cct_*` defaults.
...
The coordinator listens on `/ws/dali/devices` and `/ws/dali/groups` for immediate state. HTTP polling every 15 seconds remains the backup. Reconnect without replaying old light-changing requests. Serialize DALI traffic and avoid overlapping scans. Enable additional entities without controlling them. Scene or groups GET failures must not take lights offline. If a group record has no `dev_on`, derive on/off from member fixtures. If a member turns on while the hub still reports the group off, the group follows the members.
